...
class EligibilityService {
    static checkEligibility(user, scheme) {
        const result = {
            schemeId: scheme.id,
            scheme: scheme,
            isEligible: true,
    ...
    static checkAge(userAge, minAge, maxAge) {
    ...
    static checkGender(userGender, requiredGender) {
    ...
    static checkOccupation(userOccupation, requiredOccupations) {
    ...
    static checkIncome(userIncome, minIncome, maxIncome) {
    ...
    static checkState(userState, requiredStates) {
    ...
    static checkCustomCriteria(customCriteria) {
        return {
            status: 'partial',
    ...
    static generateRecommendations(missingCriteria, user) {
    ...
    static getEligibleSchemes(user, schemes) {
        return schemes
            .map(scheme => this.checkEligibility(user, scheme))
            .filter(result => result.isEligible)
            .sort((a, b) => b.eligibilityScore - a.eligibilityScore);
    }
    static getIneligibleSchemes(user, schemes) {
        return schemes
            .map(scheme => this.checkEligibility(user, scheme))
            .filter(result => !result.isEligible)
            .sort((a, b) => b.eligibilityScore - a.eligibilityScore);
    }
    static getAllSchemesWithEligibility(user, schemes) {
        return schemes
            .map(scheme => this.checkEligibility(user, scheme))
            .sort((a, b) => {
